// ==UserScript==
// @name         Yandex Forms Migrator Single (V9.11)
// @namespace    http://tampermonkey.net/
// @version      9.11
// @description  Исправлена привязка изображений, внедрен гибридный движок создания форм (API + Iframe 2.0)
// @author       timokaproka
...
(function() {
    ...
    function getContext() {
        ...
        return { finalId, collabId, orgId, baseUrl, createUrl, headers, isCloud };
    }
    ...
    // ==========================================
    // БЛОК 3: МОДУЛЬ БИЗНЕС-ЛОГИКИ (API И ГИБРИДНЫЙ ДВИЖОК)
    // ==========================================
    async function createFormHybrid(ctx, defaultName = "Импортированная форма") {
        console.log("[Yandex Forms Migrator Single] Запуск гибридного движка создания формы...");

        // Этап 1: Прямая API-инъекция
        try {
            console.log("[Yandex Forms Migrator Single] Попытка создания через API (/createSurvey)...");
            const payload = { name: defaultName };

            if (ctx.collabId) {
                payload.collabId = ctx.collabId;
                payload.collab_id = ctx.collabId;
            }
            if (ctx.orgId) {
                payload.orgId = ctx.orgId;
            }

            const apiRes = await fetch(window.location.origin + ctx.baseUrl + "createSurvey", {
                method: "POST",
            ...
            if (apiRes.ok) {
                ...
                if (newId) {
                    console.log(`[Yandex Forms Migrator Single] Форма успешно создана через API. ID: ${newId}`);
                    return newId;
                }
            } else {
                console.warn(`[Yandex Forms Migrator Single] API вернуло статус ${apiRes.status}`);
            }
        } catch (e) {
            console.warn("[Yandex Forms Migrator Single] Ошибка API-инъекции:", e);
        }

        // Этап 2: Iframe 2.0 (Эмуляция человека)
        console.log("[Yandex Forms Migrator Single] API недоступно. Переход на резервный метод Iframe 2.0...");
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                cleanup();
                console.error("[Yandex Forms Migrator Single] Превышено время ожидания создания формы (Iframe 2.0).");
            ...
            iframe.onload = () => {
                setTimeout(() => {
                    try {
                        ...
                        const inputTarget = doc.querySelector('input[type="text"], textarea, .g-input__control');
                        if (inputTarget) {
                            console.log("[Yandex Forms Migrator Single] Iframe 2.0: Поле ввода найдено. Эмуляция ввода...");
                            inputTarget.focus();

                            const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value")?.set;
                            const nativeTextareaValueSetter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value")?.set;

                            const setter = inputTarget.tagName === 'TEXTAREA' ? nativeTextareaValueSetter : nativeInputValueSetter;

                            if (setter) {
                                setter.call(inputTarget, defaultName);
                            } else {
                                inputTarget.value = defaultName;
                            }
                        ...
                        console.warn("[Yandex Forms Migrator Single] Iframe 2.0: Ошибка доступа к содержимому", e);
                    }
                }, 1500);
            };
            ...
            const checkInterval = setInterval(() => {
                try {
                    ...
                    if (match && !curr.includes('new-form/edit') && !curr.includes('new-form')) {
                        cleanup();
                        console.log(`[Yandex Forms Migrator Single] Форма успешно создана (Iframe 2.0). ID: ${match[0]}`);
                        resolve(match[0]);
                    }
                } catch (e) {}
            }, 500);

            iframe.src = ctx.createUrl;
        });
    }
        ...
        mainFileInput.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                reader.onload = async (ev) => {
                    try {
                        const parsedData = JSON.parse(ev.target.result);
                        const defaultName = parsedData.formName || 'Импортированная форма';
                        console.log(`[Yandex Forms Migrator Single] Файл прочитан. Подготовка к импорту формы: "${defaultName}"`);

                        const ctx = getContext();
                        const newSurveyId = await createFormHybrid(ctx, defaultName);
