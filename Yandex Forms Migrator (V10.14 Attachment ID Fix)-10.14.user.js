// ==UserScript==
// @name         Yandex Forms Migrator (V10.14 Attachment ID Fix)
...
(function() {
    ...
    async function uploadImageDirect(attachmentObj, ctx, targetSurveyId) {
        ...
        try {
            console.log(`[Yandex Forms Migrator] Загрузка стороннего изображения: ${bestUrl}`);
    ...
    // ==========================================
    // БЛОК 3: БИЗНЕС-ЛОГИКА (ЭКСПОРТ/ИМПОРТ/УДАЛЕНИЕ)
    // ==========================================
    ...
    function initUI() {
        ...
        function updateExportCounter() {
            ...
            const deleteCounterSpan = document.querySelector('.migrator-btn-delete .delete-counter');
            if (deleteCounterSpan) {
                deleteCounterSpan.innerText = `Удалить (${selectedForms.size})`;
            }
        }
        ...
        async function doBulkDelete() {
            if (selectedForms.size === 0) return;
            if (!confirm(`Вы действительно хотите удалить ${selectedForms.size} форм(ы)? Это действие необратимо.`)) return;

            const ids = Array.from(selectedForms);
            console.log(`[Yandex Forms Migrator] Запуск массового удаления. Выбрано форм: ${ids.length}.`);
            ...
            for (let i = 0; i < ids.length; i++) {
                ...
                console.log(`[Yandex Forms Migrator] Удаление формы [${i + 1}/${ids.length}], ID: ${surveyId}...`);
                try {
                    const res = await fetch(`${ctx.baseUrl}deleteSurvey`, {
                        method: "POST",
                        headers: h,
                    ...
                    if (res.ok) {
                        deletedCount++;
                        console.log(`[Yandex Forms Migrator] Форма ID ${surveyId} успешно удалена.`);
                    } else {
                        console.error(`[Yandex Forms Migrator] Ошибка удаления формы ID ${surveyId}: статус ${res.status}`);
                    }
                } catch (err) {
                    console.error(`[Yandex Forms Migrator] Ошибка сети при удалении формы ID ${surveyId}:`, err);
                }
                await sleep(300);
            }

            console.log(`[Yandex Forms Migrator] Удалено ${deletedCount} из ${ids.length} форм. Перезагрузка страницы.`);
            location.reload();
        }
        ...
        const observer = new MutationObserver(() => {
            ...
            if (filtersContainer && !document.querySelector('.migrator-bulk-actions')) {
                ...
                btnDeleteBulk.onclick = doBulkDelete;
