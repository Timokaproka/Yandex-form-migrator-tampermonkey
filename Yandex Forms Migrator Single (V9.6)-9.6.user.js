// ==UserScript==
// @name         Yandex Forms Migrator Single (V9.6)
// @namespace    http://tampermonkey.net/
// @version      9.6
// @description  Экспорт/импорт яндекс форм (исправлен перенос description и дополнительных параметров)
// @author       timokaproka
...
(function() {
    ...
    async function uploadImageDirect(attachmentObj, ctx, targetSurveyId) {
        ...
        try {
            ...
            console.warn("Ошибка загрузки изображения:", e);
            return null;
        }
    }
    ...
    async function fetchSurveyData(surveyId, ctx) {
        ...
        return {
            ...
            hooks: hooksRes // Резервное копирование интеграций
        };
    }
    ...
    async function applyData(surveyId, data, ctx) {
        ...
        try {
            ...
            console.warn("Не удалось применить основные настройки:", e);
        }
        ...
        if (data.surveyInfo) {
            try {
                ...
                console.warn("Не удалось применить расширенные настройки:", e);
            }
        }
        ...
        if (data.quizSettings) {
            try {
                ...
                console.warn("Не удалось применить настройки квиза:", e);
            }
        }
        ...
        if (data.submitConditions?.length > 0) {
            try {
                ...
                console.warn("Не удалось применить условия отправки:", e);
            }
        }
            ...
            if (Array.isArray(newHooks)) {
                for (const hook of newHooks) {
                    if (hook && hook.id) {
                        ...
                        await sleep(200); // Задержка для предотвращения блокировки по Rate Limit
                    }
                }
            }
        } catch (e) {
            console.warn("Не удалось отключить дефолтные уведомления:", e);
        }
            ...
            if (file) {
                ...
                reader.onload = async (ev) => {
                    try {
                        ...
                        if (surveyId) {
                            await applyData(surveyId, JSON.parse(ev.target.result), ctx);
            ...
            if (file) {
                ...
                reader.onload = async (ev) => {
                    try {
                        ...
                        if (newSurveyId) {
                            await applyData(newSurveyId, JSON.parse(ev.target.result), ctx);
